...
for dur in durs:
    for sync in syncs:
        if sync == "sync":
            title_sync = "synchronised only"
        else:
            title_sync = "non-synchronised included"
        ...
        for osc_idx, osc in enumerate(osc_types):
            ...
            for cond_idx, cond in enumerate(conds):
                epo = epochs["Cond=='{}{}'".format(cond, dur)]
                ...
                circ_mean = np.rad2deg(circmean(SI_ang))
                circ_std = np.rad2deg(circstd(SI_ang))
                ...
                if len(osc_types) > 1:
                    circular_hist(axes[osc_idx][cond_idx], SI_ang, color=colors[cond_idx],
                                  alpha=0.6)
                    axes[osc_idx][cond_idx].set_title("{} {}\nMean {:.1f}, Std {:.1f}\nRayleigh: p={:.3f}, V-test (0): p={:.3f}".format(osc, title_keys[cond], circ_mean, circ_std, rayleigh, vt))
                else:
                    circular_hist(axes[cond_idx], SI_ang, color=colors[cond_idx],
                                  alpha=0.6)
                    axes[cond_idx].set_title("{} {}\nMean {:.3f}, Std {:.3f}\nRayleigh: p={:.3f}, V-test (0): p={:.3f}".format(osc, title_keys[cond], circ_mean, circ_std, rayleigh, vt))
        plt.suptitle("{}, {}\n".format(dur, title_sync))
        plt.tight_layout()
        plt.savefig("SI_{}_{}.tif".format(dur, sync))
