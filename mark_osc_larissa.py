    ...
    if this_match:
        ...
        for minmax_freq, minmax_time, osc_type in zip(minmax_freqs,
            ...
            for k in chan_groups.keys():
                ...
                mark_osc_amp(osc_events, amp_thresh, k, minmax_times, osc_type,
                             raw_inst=raw_work)
                ...
                for event in np.nditer(events[0][:,-1]):
                    eve = event.copy()
                    if eve >= 100:
                        df_dict["PureIndex"].append(eve//100)
                        eve = eve%100
                    else:
                        df_dict["PureIndex"].append(0)
                    if eve >= 50:
                ...
                epo = mne.Epochs(raw, events[0], tmin=-1.5, tmax=1.5, detrend=None,
