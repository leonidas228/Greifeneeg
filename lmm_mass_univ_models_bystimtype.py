        ...
        for sync_fact in sync_facts:
            for stim_type in stim_types:
                ...
                if sync_fact == "syncfact":
                    formula = "Brain ~ C(Dur, Treatment('30s'))*C(PrePost, Treatment('Pre'))*C(Sync, Treatment('sync'))"
                else:
                    formula = "Brain ~ C(Dur, Treatment('30s'))*C(PrePost, Treatment('Pre'))"
                outfile = "{}main_fits_{}_grand_{}_{}_{}_{}_{}.pickle".format(proc_dir, baseline, stim_type, osc, bs_name, use_group, sync_fact)
